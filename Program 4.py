...
class RockPaperScissorsGame:
    def __init__(self, master):
        ...
        # Instructions label
        #Use sticky="nsew" for felixibility of the buttons with respect to windows.
        self.label = tk.Label(master, text="Choose Rock, Paper, or Scissors:")
        self.label.grid(row=0, column=0, columnspan=3, pady=(10, 5), sticky="nsew")

        # Buttons for user choices
        self.rock_button = tk.Button(master, text="Rock", command=lambda: self.play("rock"), width=10)
        self.rock_button.grid(row=1, column=0, padx=5, pady=5, sticky="nsew")

        self.paper_button = tk.Button(master, text="Paper", command=lambda: self.play("paper"), width=10)
        self.paper_button.grid(row=1, column=1, padx=5, pady=5, sticky="nsew")

        self.scissors_button = tk.Button(master, text="Scissors", command=lambda: self.play("scissors"), width=10)
        self.scissors_button.grid(row=1, column=2, padx=5, pady=5, sticky="nsew")

        # Result label
        self.result_label = tk.Label(master, text="", wraplength=300)
        self.result_label.grid(row=2, column=0, columnspan=3, pady=(5, 5), sticky="nsew")

        # Score label
        self.score_label = tk.Label(master, text="User: 0 | Computer: 0")
        self.score_label.grid(row=3, column=0, columnspan=3, pady=(5, 10), sticky="nsew")

        # Play Again button
        self.play_again_button = tk.Button(master, text="Play Again", command=self.reset_game, state=tk.DISABLED)
        self.play_again_button.grid(row=4, column=0, columnspan=3, pady=(5, 5), sticky="nsew")

        # Exit button
        self.exit_button = tk.Button(master, text="Exit", command=master.quit)
        self.exit_button.grid(row=5, column=0, columnspan=3, pady=(5, 10), sticky="nsew")
    ...
    def play(self, user_choice):
        ...
        if self.user_score >= self.max_score or self.computer_score >= self.max_score:
            self.result_label.config(text=f"{result} Final Scores - User: {self.user_score}, Computer: {self.computer_score}")
